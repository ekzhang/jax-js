...
suite.each(devices)("device:%s", (device) => {
  ...
  if (device === "wasm") {
    ...
    test("retains buffers until asynchronous worker dispatch completes", async () => {
      const backend = getBackend(device);
      const blockerSize = 2_000_000;
      const victimSize = 4096;
      const blocker = backend.malloc(blockerSize * 4);
      const victimData = new Float32Array(victimSize);
      const clobberData = new Float32Array(victimSize);
      for (let i = 0; i < victimSize; i++) {
        victimData[i] = i + 1;
        clobberData[i] = -1000 - i;
      }
      const victim = backend.malloc(victimSize * 4, new Uint8Array(victimData.buffer));
      const output = backend.malloc(victimSize * 4);
      const clobbers: number[] = [];

      try {
        const blockerKernel = await backend.prepareKernel(
          new Kernel(
            0,
            blockerSize,
            AluExp.sin(AluExp.cast(DType.Float32, AluVar.gidx)),
          ),
        );
        const victimKernel = await backend.prepareKernel(
          new Kernel(
            1,
            victimSize,
            AluExp.add(
              accessorGlobal(
                DType.Float32,
                0,
                ShapeTracker.fromShape([victimSize]),
                [AluVar.gidx],
              ),
              AluExp.f32(1),
            ),
        ...
        backend.dispatch(blockerKernel, [], [blocker]);
        backend.dispatch(victimKernel, [victim], [output]);

        // Frontend PendingExecute releases its slot refs immediately after
        // dispatch. The wasm backend must keep its own refs until queued worker
        // execution has really finished, otherwise this allocation can reuse
        // victim's pointer before victimKernel reads it.
        backend.decRef(victim);
        for (let i = 0; i < 64; i++) {
          clobbers.push(
            backend.malloc(victimSize * 4, new Uint8Array(clobberData.buffer)),
          );
        }

        const { buffer } = await backend.read(output);
        const actual = new Float32Array(buffer);
        for (let i = 0; i < victimSize; i++) {
          expect(actual[i]).toBe(victimData[i] + 1);
        }
      } finally {
        backend.decRef(blocker);
        backend.decRef(output);
        for (const slot of clobbers) backend.decRef(slot);
      }
    });
  }
